...
static const FridaSELinuxRule frida_selinux_rules[] =
{
  ...
  { { "domain", NULL }, "mhook_file", "dir", { "search", NULL } },
  { { "domain", NULL }, "mhook_file", "file", { "open", "read", "getattr", "execute", "?map", NULL } },
  { { "domain", NULL }, "mhook_memfd", "file", { "open", "read", "write", "getattr", "execute", "?map", NULL } },
...
frida_selinux_patch_policy (void)
{
  ...
  if (frida_ensure_type (&db, "mhook_file", 2, "file_type", "mlstrustedobject", &error) == NULL)
  {
  ...
  if (frida_ensure_type (&db, "mhook_memfd", 2, "file_type", "mlstrustedobject", &error) == NULL)
  {
    g_printerr ("Unable to add SELinux type: %s\n", error->message);
    g_clear_error (&error);
    goto beach;
