...
frida_gadget_environment_init (void)
{
  ...
  worker_thread = g_thread_new ("mhook", run_worker_loop, NULL);
}
...
frida_parse_apple_parameters (const gchar * apple[], gboolean * found_range, GumMemoryRange * range, gchar ** config_data)
{
  ...
  *config_data = NULL;
  ...
  {
    if (g_str_has_prefix (entry, "mhook_dylib_range="))
    {
      *found_range = sscanf (entry, "mhook_dylib_range=0x%" G_GINT64_MODIFIER "x,0x%" G_GSIZE_MODIFIER "x",
          &range->base_address, &range->size) == 2;
    }
    else if (g_str_has_prefix (entry, "mhook_gadget_config="))
    {
      guchar * data;
      gsize size;
